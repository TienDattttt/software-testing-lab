...
short src_ln1 [] = {
	  0,  39,  18,  18,  18,  16,  41,  42, 
	 44,  45,  46,  47,  21,  21,  20,  38, 
	 50,  38,  50,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "pc.pml", 1, 18 },
...
short src_ln0 [] = {
	  0,  25,  18,  18,  18,  16,  27,  28, 
	 30,  31,  32,  33,  21,  21,  20,  24, 
	 36,  24,  36,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "pc.pml", 1, 18 },
...
#ifndef NOCLAIM
	case 6:	/* claim select */
		spin_c_typ[0] = 4; /* producer_no_overwrite */
		((P6 *)pptr(h))->c_cur[0] = 6;
		reached4[6]=1;
		spin_c_typ[1] = 3; /* non_starvation */
		((P6 *)pptr(h))->c_cur[1] = 5;
		reached3[5]=1;
		spin_c_typ[2] = 2; /* mutual_exclusion */
		((P6 *)pptr(h))->c_cur[2] = 6;
		reached2[6]=1;
		((P6 *)pptr(h))->_t = 2;
		((P6 *)pptr(h))->_p = 6;
		((P6 *)pptr(h))->_n = 2; /* mutual_exclusion */
		src_claim = src_ln2;
#ifndef BFS
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("in_cs[l_in]", now.in_cs[l_in]);
		}
	}
		logval("prod_evt", now.prod_evt);
		logval("cons_evt", now.cons_evt);
		logval("prod_pre_flag", now.prod_pre_flag);
#endif
}
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	bit    in_cs[%d]:	%d\n", l_in, now.in_cs[l_in]);
		}
	}
	printf("	bit    prod_evt:	%d\n", now.prod_evt);
	printf("	bit    cons_evt:	%d\n", now.cons_evt);
	printf("	bit    prod_pre_flag:	%d\n", now.prod_pre_flag);
}
void
